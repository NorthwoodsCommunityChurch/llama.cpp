...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_sum_rows(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    {
        const int NW = ggml_metal_library_get_simd_width(lib);
        res.smem = NW*sizeof(float);
    }
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_soft_max(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    res.smem = ggml_metal_library_get_simd_width(lib)*sizeof(float);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_solve_tri(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    res.nsg  = nsg;
    res.smem = GGML_PAD(GGML_PAD(n, ggml_metal_library_get_simd_width(lib))*nsg*sizeof(float), 16);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_mul_mv(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    switch (tsrc0) {
        ...
        case GGML_TYPE_BF16:
            {
                if (ne00 < ggml_metal_library_get_simd_width(lib)) {
                    nsg = 1;
                    nr0 = 32;
                    nr1 = 1;
                    suffix = "_short";
                    ...
                    nr0 = 2;
                    nr1 = 1;
                    smem = ggml_metal_library_get_simd_width(lib)*sizeof(float)*nr0;
                    suffix = ne00 % 4 == 0 ? "_4" : "";
                }
            } break;
        case GGML_TYPE_Q4_0:
        ...
        case GGML_TYPE_Q8_0:
            {
                nsg = N_SG_Q8_0;
                nr0 = N_R0_Q8_0;
                smem = ggml_metal_library_get_simd_width(lib)*sizeof(float)*N_R0_Q8_0;
            } break;
        case GGML_TYPE_MXFP4:
            {
                nsg = N_SG_MXFP4;
                nr0 = N_R0_MXFP4;
                smem = ggml_metal_library_get_simd_width(lib)*sizeof(float);
            } break;
        case GGML_TYPE_Q2_K:
            {
                nsg = N_SG_Q2_K;
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_argmax(ggml_metal_library_t lib, const ggml_tensor * op) {
    ...
    res.smem = ggml_metal_library_get_simd_width(lib)*(sizeof(float) + sizeof(int32_t));
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_count_equal(ggml_metal_library_t lib, const ggml_tensor *  op) {
    ...
    const int NW_ce = ggml_metal_library_get_simd_width(lib);
    int nsg = 1;
    while (NW_ce*nsg < ne00 && nsg < 32) {
        nsg *= 2;
    }
    ...
    res.smem = ggml_metal_library_get_simd_width(lib) * sizeof(int32_t);
    res.nsg  = nsg;
